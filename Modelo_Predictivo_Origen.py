...
uploaded = st.file_uploader("Sube tu archivo Excel o CSV (Fecha, Valor)",
                             type=["xlsx","xls","csv"])

steps_horizon = st.number_input("Pasos futuros a pronosticar",
...
metricas = pd.DataFrame({
    ...
    "RMSE":   [rmse(test_series,sarima_wf),
...
# ---------- Forecast futuro ----------------------------------------
fc_df = forecast_multimodelo(series, m, freq,
                             steps=steps_horizon,
...
# ---------- Botones de descarga ------------------------------------
st.download_button("Descargar predicciones_walk_forward.xlsx",
                   buf1.getvalue(),
                   file_name="predicciones_walk_forward.xlsx")

st.download_button("Descargar forecast_multimodelo.xlsx",
                   buf2.getvalue(),
