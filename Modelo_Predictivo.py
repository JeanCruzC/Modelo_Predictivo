...
# ------------------------------------------------------------------
# Funciones utilitarias (sin caché)
# ------------------------------------------------------------------
    ...
    for idx in test.index:
        try:
            model = auto_arima(history,
                               ...
                               random_state=42)
            p = float(model.predict(n_periods=1)[0])
        except Exception:
...
def forecast_multimodelo(series, m, freq, steps, rf_model, xgb_model):
    ...
    try:
        sarima_model = auto_arima(series,
                                  ...
                                  suppress_warnings=True,
                                  random_state=42)
        sarima_fc = sarima_model.predict(n_periods=steps).astype(float)
    except Exception:
    ...
    try:
        arima_model = auto_arima(series,
                                 ...
                                 suppress_warnings=True,
                                 random_state=42)
        arima_fc = arima_model.predict(n_periods=steps).astype(float)
    except Exception:
    ...
    # Características futuras para ML
    ...
    for dt in fechas:
        ...
        feats.append([dt.month, dt.dayofyear, lag1, lag2, mv99003])
...
metricas = pd.DataFrame({
    "Modelo": ["ARIMA_WFV","SARIMA_WFV","WES_WFV",
