...
# ------------------------------------------------------------------
# Decoradores de cache para modelos y datos
# ------------------------------------------------------------------
@st.cache_data
def load_data(uploaded):
    # Lee Excel o CSV y prepara la serie
...
@st.cache_resource
def train_ml_models(series_index, feat_df):
    # Entrena RF y XGB una sola vez por dataset
    split = int(len(feat_df) * 0.8)
    X_train = feat_df.iloc[:split][['Mes','DiaDelAnio','Lag1','Lag2','MediaMovil3']]
    y_train = feat_df.iloc[:split]['Valor']

    rf = GridSearchCV(
        RandomForestRegressor(random_state=42),
        {"n_estimators":[100,200], "max_depth":[5,None]}, cv=3
    ).fit(X_train, y_train)

    xgb = GridSearchCV(
        XGBRegressor(random_state=42, verbosity=0),
        {"n_estimators":[100], "learning_rate":[0.05]}, cv=3
    ).fit(X_train, y_train)
...
def forecast_multimodelo(series, m, freq, steps, rf, xgb):
    ...
    return pd.DataFrame({
        'SARIMA':sarima_fc,'ARIMA':arima_fc,'WES':wes_fc,
        'RandomForest':rf_fc,'XGBoost':xgb_fc,'Stacking':stack_fc
    }, index=fechas)
...
ml_df = pd.DataFrame({
    'RandomForest':rf_pred,'XGBoost':xgb_pred,'Stacking':stack_pred
}, index=y_test.index)
...
# Forecast futurofc = forecast_multimodelo(series, m, freq, steps, rf, xgb)
...
# Descargas\st.download_button('Descargar predicciones_walk_forward.xlsx', buf1.getvalue())
