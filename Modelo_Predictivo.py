...
# --- Carga de datos -----------------------------------------------------
uploaded = st.file_uploader("Sube tu archivo Excel o CSV con datos de serie temporal", type=["xlsx", "csv"])
if not uploaded:
    st.stop()
...
# --- Definir horizontes --------------------------------------------------
pasos_forecast = st.number_input("¿Cuántos pasos hacia adelante quieres pronosticar?", min_value=1, value=6)

# --- Selección de modelos ------------------------------------------------
all_models = ["SARIMA", "ARIMA", "WES", "RandomForest", "XGBoost", "Stacking"]
select_all = st.checkbox("Incluir todos los modelos", value=True)
selected_models = all_models if select_all else st.multiselect("Selecciona modelos", all_models, default=all_models)
